...
func (pm *Manager) Acquire(name string, port int) (realPort int, err error) {
	return pm.acquire(name, port, nil)
}

func (pm *Manager) AcquireFromRanges(name string, port int, allowPorts []types.PortsRange) (realPort int, err error) {
	return pm.acquire(name, port, allowPorts)
}

func (pm *Manager) acquire(name string, port int, allowPorts []types.PortsRange) (realPort int, err error) {
	portCtx := &PortCtx{
		ProxyName:  name,
		Closed:     false,
	...
	if port == 0 {
		...
		for k := range pm.freePorts {
			if len(allowPorts) > 0 && !portInRanges(k, allowPorts) {
				continue
			}
			count++
		...
		// specified port
		if len(allowPorts) > 0 && !portInRanges(port, allowPorts) {
...
func portInRanges(port int, ranges []types.PortsRange) bool {
	for _, pair := range ranges {
		...
		if port >= pair.Start && port <= pair.End {
			return true
		}
	}
	return false
