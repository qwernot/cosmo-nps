...
func NewService(cfg *v1.ServerConfig) (*Service, error) {
	...
	if cfg.UserStore.Enable {
		userStore, err = userstore.NewStore(cfg.UserStore)
		if err != nil {
			return nil, err
		}
	}
	if cfg.WebServer.Port > 0 {
		...
		if userStore != nil {
			ws.SetAuthMiddleware(userStore.AuthMiddleware)
		}
		webServer = ws
...
func (svr *Service) UpsertUserStoreUser(in userstore.User) error {
	if svr.userStore == nil {
		return fmt.Errorf("userStore is not enabled")
	}
	_, err := svr.userStore.Upsert(in)
	return err
}

func (svr *Service) DeleteUserStoreUser(name string) error {
	if svr.userStore == nil {
		return fmt.Errorf("userStore is not enabled")
	}
	return svr.userStore.Delete(name)
}

func (svr *Service) ListUserStoreUsers() ([]userstore.PublicUser, error) {
	if svr.userStore == nil {
		return nil, fmt.Errorf("userStore is not enabled")
	}
...
func (svr *Service) handleConnection(ctx context.Context, conn net.Conn, internal bool) {
	...
	case *msg.Login:
		...
		if err == nil {
			m = &retContent.Login
			authVerifier, encryptionKey, authErr := svr.loginAuth(m, internal)
			if authErr != nil {
				err = authErr
			}
			...
			if err == nil && !internal {
				...
				controlRW, err = acceptedConn.newControlReadWriter(conn, encryptionKey)
				if err == nil {
					controlConn = acceptedConn.messageConnFor(controlRW)
				}
			}
			if err == nil {
				ctl, err = svr.RegisterControl(controlConn, m, internal, acceptedConn.wireProtocol, authVerifier, encryptionKey)
			}
		}
...
func (svr *Service) loginAuth(loginMsg *msg.Login, internal bool) (auth.Verifier, []byte, error) {
	if internal && loginMsg.ClientSpec.AlwaysAuthPass {
		return auth.AlwaysPassVerifier, svr.auth.EncryptionKey(), nil
	}
	if svr.userStore != nil {
		return svr.userStore.ClientAuth(loginMsg, svr.cfg.Auth.AdditionalScopes)
	}
	if err := svr.auth.Verifier.VerifyLogin(loginMsg); err != nil {
		return nil, nil, err
	}
	return svr.auth.Verifier, svr.auth.EncryptionKey(), nil
}
