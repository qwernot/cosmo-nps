...
func (pxy *UDPProxy) Run() (remoteAddr string, err error) {
	...
	if pxy.sessionUserStoreEnabled() {
		ports := pxy.sessionUserPorts()
		if len(ports) == 0 {
			return "", fmt.Errorf("user [%s] has no assigned udp port pool", pxy.GetUserInfo().User)
		}
		pxy.realBindPort, err = pxy.rc.UDPPortManager.AcquireFromRanges(
			pxy.name,
			pxy.cfg.RemotePort,
			ports,
		)
