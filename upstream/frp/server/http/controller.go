...
func (c *Controller) currentUser(ctx *httppkg.Context) sessionInfo {
	if c.userStore == nil {
		return sessionInfo{Role: userstore.RoleAdmin, IsAdmin: true}
	}
	u, ok := userstore.FromContext(ctx.Req.Context())
	if !ok {
		return sessionInfo{}
	}
	return sessionInfo{
		Name:    u.Name,
		Role:    u.Role,
		IsAdmin: u.Role == userstore.RoleAdmin,
	}
}

func (c *Controller) requireAdmin(ctx *httppkg.Context) error {
	if c.currentUser(ctx).IsAdmin {
		return nil
	}
	return httppkg.NewError(http.StatusForbidden, "admin permission required")
}

func (c *Controller) canReadUser(ctx *httppkg.Context, owner string) bool {
	session := c.currentUser(ctx)
	return session.IsAdmin || session.Name == owner
}

func (c *Controller) APISession(ctx *httppkg.Context) (any, error) {
	return c.currentUser(ctx), nil
}
...
func (c *Controller) APIClientList(ctx *httppkg.Context) (any, error) {
	...
	if session := c.currentUser(ctx); !session.IsAdmin {
		userFilter = session.Name
	}
...
func (c *Controller) APIClientDetail(ctx *httppkg.Context) (any, error) {
	...
	if !c.canReadUser(ctx, info.User) {
		return nil, httppkg.NewError(http.StatusForbidden, "permission denied")
	}
...
func (c *Controller) APIProxyByType(ctx *httppkg.Context) (any, error) {
	...
	if session := c.currentUser(ctx); !session.IsAdmin {
		proxyInfoResp.Proxies = slices.DeleteFunc(proxyInfoResp.Proxies, func(item *model.ProxyStatsInfo) bool {
			return item.User != session.Name
		})
	}
...
func (c *Controller) APIProxyByTypeAndName(ctx *httppkg.Context) (any, error) {
	...
	if !c.canReadUser(ctx, proxyStatsResp.User) {
		return nil, httppkg.NewError(http.StatusForbidden, "permission denied")
	}
