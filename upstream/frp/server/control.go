...
type SessionContext struct {
	...
	EncryptionKey []byte
	// user store manages frps web users and per-user port pools.
	UserStore interface {
...
func (ctl *Control) RegisterProxy(pxyMsg *msg.NewProxy) (remoteAddr string, err error) {
	...
	pxy, err := proxy.NewProxy(ctl.ctx, &proxy.Options{
		...
		UserPorts:          ctl.sessionCtx.UserStore,
	})
	if err != nil {
		return remoteAddr, err
	}

	maxPorts := int(ctl.sessionCtx.ServerCfg.MaxPortsPerClient)
	if ctl.sessionCtx.UserStore != nil {
		maxPorts = ctl.sessionCtx.UserStore.UserMaxPorts(ctl.sessionCtx.LoginMsg.User)
	}

	// Check ports used number in each client.
	if maxPorts > 0 {
		ctl.mu.Lock()
		if ctl.portsUsedNum+pxy.GetUsedPortsNum() > maxPorts {
			ctl.mu.Unlock()
			err = fmt.Errorf("exceed the max ports for user")
			return
		}
...
func (ctl *Control) CloseProxy(closeMsg *msg.CloseProxy) (err error) {
	...
	if ctl.sessionCtx.ServerCfg.MaxPortsPerClient > 0 || ctl.sessionCtx.UserStore != nil {
		ctl.portsUsedNum -= pxy.GetUsedPortsNum()
	}
