...
type ServerConfig struct {
	...
	UserStore UserStoreConfig  `json:"userStore,omitempty"`
...
type UserStoreConfig struct {
	// Enable turns on the built-in frps user and port-pool manager.
	// When enabled, frpc logins are authenticated by userStore records instead
	// of the single global auth.token.
	Enable bool `json:"enable,omitempty"`
	// Path stores users, tokens and port pools. If empty, ./data/frps-users.json is used.
	Path string `json:"path,omitempty"`
	// AdminUser is used only to bootstrap an empty store.
	AdminUser string `json:"adminUser,omitempty"`
	// AdminPassword is used only to bootstrap an empty store.
	AdminPassword string `json:"adminPassword,omitempty"`
}

func (c *UserStoreConfig) Complete() {
	c.Path = util.EmptyOr(c.Path, "./data/frps-users.json")
	c.AdminUser = util.EmptyOr(c.AdminUser, "admin")
}
