...
func (s *IndexController) getRequestClient() (*file.Client, error) {
	clientId := s.GetIntNoErr("client_id")
	if isAdmin := s.GetSession("isAdmin"); isAdmin != nil && !isAdmin.(bool) {
		if sessionClientId := s.GetSession("clientId"); sessionClientId != nil {
			clientId = sessionClientId.(int)
		}
	}
	return file.GetDb().GetClient(clientId)
}

func (s *IndexController) getClientPortPool(client *file.Client) []int {
	if client == nil || client.PortPool == "" {
		return nil
	}
	return common.GetPorts(client.PortPool)
}

func (s *IndexController) ensureTunnelPort(t *file.Tunnel, testListen bool) bool {
	pool := s.getClientPortPool(t.Client)
	if t.Port <= 0 {
		t.Port = tool.GenerateServerPortInPool(t.Mode, pool)
		if t.Port <= 0 {
			s.AjaxErr("No available server port in this user's port pool")
			return false
		}
	}
	if !tool.IsServerPortAllowed(t.Port, t.Mode, pool) {
		s.AjaxErr("The port is outside this user's port pool.")
		return false
	}
	if testListen && !tool.TestServerPortInPool(t.Port, t.Mode, pool) {
		s.AjaxErr("The port cannot be opened because it may has been occupied or is outside this user's port pool.")
		return false
	}
	return true
}
...
func (s *IndexController) Add() {
	if s.Ctx.Request.Method == "GET" {
		...
		var err error
		if t.Client, err = s.getRequestClient(); err != nil {
			s.AjaxErr(err.Error())
		}
		if !s.ensureTunnelPort(t, true) {
			return
		}
...
func (s *IndexController) Copy() {
	...
	if oldTask, err := file.GetDb().GetTask(oldId); err != nil {
		...
		if !s.ensureTunnelPort(newTask, true) {
			return
		}
...
func (s *IndexController) Edit() {
	...
	if s.Ctx.Request.Method == "GET" {
		...
		if t, err := file.GetDb().GetTask(id); err != nil {
			...
			oldPort := t.Port
			oldMode := t.Mode
			if client, err := s.getRequestClient(); err != nil {
				s.AjaxErr("modified error,the client is not exist")
				return
			} else {
				t.Client = client
			...
			if !s.ensureTunnelPort(t, t.Port != oldPort || t.Mode != oldMode) {
				return
			}
