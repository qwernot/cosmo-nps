...
func TestServerPort(p int, m string) (b bool) {
	return TestServerPortInPool(p, m, nil)
}

func TestServerPortInPool(p int, m string, pool []int) (b bool) {
	if !IsServerPortAllowed(p, m, pool) {
		return false
	}
	if m == "udp" {
...
func IsServerPortAllowed(p int, m string, pool []int) bool {
	if m == "p2p" || m == "secret" {
		return true
	}
	if p > 65535 || p < 0 {
	...
	if len(pool) != 0 && !common.InIntArr(pool, p) {
		return false
	}
	if len(ports) != 0 {
...
func GenerateServerPort(m string) int {
	return GenerateServerPortInPool(m, nil)
}

func GenerateServerPortInPool(m string, pool []int) int {
	if len(pool) != 0 {
		for _, serverPort := range pool {
			if TestServerPortInPool(serverPort, m, pool) {
				return serverPort
			}
		}
		return 0
