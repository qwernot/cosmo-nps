...
Tunnel Control 是一个 NPS 多节点统一管理面板。当前新项目只做 NPS：

- 总控 `darkver8/tunnel-all:latest`：只运行 Web 后台，管理用户、端口池、域名池、节点和隧道。
- 节点 `darkver8/tunnel-port:latest`：运行 `tunnel-agent`，启动 NPS 服务并接收总控下发配置。
- 用户客户端 `darkver8/tunnel-port:latest`：运行 `tunnel-client`，只连接总控，再自动连接对应节点的 NPS bridge。
...
`darkver8/tunnel-port:latest` 同时包含两个入口：

```text
/usr/local/bin/tunnel-agent   # 节点服务
/usr/local/bin/tunnel-client  # 用户客户端
```
...
用户侧推荐运行 `tunnel-client`。用户只需要填写总控地址、后台用户名和后台密码，不需要知道具体应该连接哪个节点。
...
3. 创建节点，复制节点 Token。
4. 在节点服务器部署 `tunnel-agent`。
5. 在后台给用户创建 NPS 隧道，选择目标节点。
6. 用户侧运行 `tunnel-client`，只连接总控即可。
